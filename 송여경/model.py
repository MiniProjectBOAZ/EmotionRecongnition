# CM-KoBERT 의 모델 부분 코드
# finetune layer = masked attention / classification layer 등이 기존 BERT와 다름.
...
class BertFinetun(nn.Module):
    def __init__(self, config):
        ...
        self.proj_a = nn.Conv1d(465, 30, kernel_size=1, padding=0, bias=False)
...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(self, input_ids, all_audio_data, token_type_ids=None, attention_mask=None, labels=None):
        ...
        if labels is not None: 
            loss_fct = CrossEntropyLoss() 
            loss = loss_fct(logits.view(-1, 7), labels.long())
            return loss
